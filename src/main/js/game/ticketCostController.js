define([
...
], function (Sprite, msgBus, audio, gr, loader, SKBeInstant, gladButton, gameUtils, config, revealDataSaveController) {
    ...
    function onGameParametersUpdated() {
		...
		var arrowPlusType = {'scaleXWhenClick': -0.92, 'scaleYWhenClick': 0.92};
    ...
    function increaseTicketCost() {
        ...
        if(index === prizePointList.length-1){
			...
			var channel = audioChannelArray[index % 3];
            audio.play('ButtonBetUp', channel);
        }       
    }
    ...
    function decreaseTicketCost() {
        ...
        setTicketCostValueWithNotify(prizePointList[index]);
		var channel = audioChannelArray[index % 3]; 
        audio.play('ButtonBetDown', channel);
    }
    ...
    function onStartUserInteraction(data) {
		...
		if(SKBeInstant.config.wagerType === 'BUY'&&SKBeInstant.config.gameType === 'ticketReady'&&hasRevealData(data)){
			...
			if(revealData&&revealData.prizePoint){
				_currentPrizePoint = revealData.prizePoint;
				setTicketCostValueWithNotify(_currentPrizePoint);
			}
		}
    }
